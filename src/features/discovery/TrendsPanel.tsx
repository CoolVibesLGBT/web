...
const darkRankStyles: Array<{ background: string; color: string }> = [
  {
...
const getRankStyle = (rank: number, isDark = false) => {
  const rankStyles = isDark ? darkRankStyles : rainbowRankStyles;

  if (rank <= rankStyles.length) {
    return rankStyles[rank - 1];
  }

  if (isDark) {
    return {
      background: 'linear-gradient(135deg, rgba(14, 165, 233, 0.16) 0%, rgba(56, 189, 248, 0.08) 100%)',
      color: '#bae6fd',
    };
  }
...
const TrendsPanel: React.FC<TrendsPanelProps> = ({ limit = 20, onTrendSelect }) => {
  ...
  const subtitleColor = isDark ? 'text-zinc-400' : 'text-gray-500';
  const cardBackground =
    isDark
      ? 'cv-card-surface-soft border border-white/10 shadow-[0_24px_70px_-48px_rgba(0,0,0,0.9)] backdrop-blur-3xl'
      : 'border border-white/70 bg-gradient-to-br from-white via-white/97 to-gray-50 shadow-[0_24px_70px_-48px_rgba(15,23,42,0.35)]';
  ...
  return (
    <motion.div
      ...
      className={`overflow-hidden rounded-3xl ${cardBackground}`}
    >
      <div className="p-5 pb-4 flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <TrendingUp className={`h-5 w-5 ${isDark ? 'text-sky-300' : 'text-sky-600'}`} />
          ...
          {lastUpdated && (
            <div className="mt-3 flex items-center gap-2">
              <span
                className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold ${isDark ? 'bg-sky-400/10 text-sky-200 ring-1 ring-inset ring-sky-300/10' : 'bg-gray-100 text-gray-600'
                  }`}
              >
              ...
              {trends.length > 0 && (
                <span
                  className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold ${isDark ? 'bg-white/[0.05] text-zinc-400 ring-1 ring-inset ring-white/[0.06]' : 'bg-gray-50 text-gray-600'
                    }`}
                >
                  #{trends.length}
                </span>
      ...
      <div className="px-5 pb-5 space-y-3">
        {error ? (
          <div
            className={`rounded-2xl border p-4 ${isDark
              ? 'cv-card-surface-muted border-rose-400/20 text-rose-200 shadow-[0_14px_32px_-22px_rgba(251,113,133,0.45)]'
          ...
          >
            ...
            <button
              type="button"
              onClick={fetchTrends}
              className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${isDark
          ...
          <>
              ...
              {loading && trends.length === 0 ? (
                ...
                >
                  {Array.from({ length: Math.min(6, limit) }).map((_, index) => (
                    <div
                      key={`skeleton-${index}`}
                      className={`flex items-center gap-3 rounded-2xl border px-3 py-3 animate-pulse ${isDark ? 'cv-card-surface-muted border-white/[0.06]' : 'border-transparent bg-gray-100/70'
                        }`}
                    >
                      <div
                        className={`h-6 w-6 rounded-full ${isDark ? 'bg-white/[0.08]' : 'bg-gray-200'
                          }`}
                      />
                      <div className="flex-1 space-y-2">
                        <div className={`h-3 w-2/3 rounded ${isDark ? 'bg-white/[0.08]' : 'bg-gray-200'}`} />
                        <div className={`h-3 w-1/3 rounded ${isDark ? 'bg-white/[0.08]' : 'bg-gray-200'}`} />
                      </div>
                      <div
                        className={`h-3 w-12 rounded ${isDark ? 'bg-white/[0.08]' : 'bg-gray-200'}`}
                      />
                    </div>
                  ))}
                </motion.div>
                  ...
                  {displayedTrends.map((trend) => {
                    ...
                    const changeLabel =
                      typeof trend.change === 'number' && Number.isFinite(trend.change)
                        ...
                        : null;
                    const rankStyle = getRankStyle(trend.rank, isDark);
                    ...
                    return (
                      <motion.button
                        ...
                        className={`group grid cursor-pointer grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border px-3 py-2.5 transition-all duration-200 ${isDark
                          ? 'cv-card-surface-muted border-white/[0.06] shadow-[0_12px_28px_-24px_rgba(0,0,0,0.9)] hover:border-sky-300/20 hover:bg-white/[0.075]'
                          : 'border-slate-200/70 bg-white hover:bg-gray-50'
                          }`}
                      >
                        <span
                          style={rankStyle}
                        ...
                        <div className="min-w-0 text-left">
                          <p className={`truncate text-sm font-semibold leading-none ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {trend.label}
                          </p>
                          {trend.subtitle && (
                        ...
                        <div className="flex items-center gap-2 justify-self-end">
                          {volumeLabel && (
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${isDark ? 'bg-sky-400/10 text-sky-200 ring-1 ring-inset ring-sky-300/10' : 'bg-gray-100 text-gray-600'
                                }`}
                            >
                          ...
                          <ArrowUpRight
                            className={`h-4 w-4 flex-shrink-0 transition-transform duration-200 ${isDark
                              ? 'text-zinc-500 group-hover:translate-x-1 group-hover:text-sky-300'
            ...
            {hasOverflow && (
              <div className="flex">
                <button
                  ...
                  className={`ml-auto inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${isDark
                    ? 'bg-sky-400/10 text-sky-200 ring-1 ring-inset ring-sky-300/10 hover:bg-sky-400/20'
            ...
            {!loading && trends.length === 0 && (
              <div
                className={`rounded-2xl border p-6 text-center text-sm ${isDark ? 'cv-card-surface-muted border-white/[0.06] text-zinc-400' : 'border-transparent bg-gray-100/60 text-gray-500'
                  }`}
              >
        ...
        <button
          ...
          className={`inline-flex w-full items-center justify-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-colors ${isDark
            ? 'bg-sky-500 text-white shadow-[0_12px_28px_-18px_rgba(14,165,233,0.95)] hover:bg-sky-400'
