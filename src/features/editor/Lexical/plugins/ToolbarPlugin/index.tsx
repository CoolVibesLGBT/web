...
import { DEFAULT_SANS_FONT_FAMILY } from '../../../../../constants/constants';
...
const FONT_FAMILY_OPTIONS: [string, string][] = [
  [DEFAULT_SANS_FONT_FAMILY, 'Google Sans'],
];
...
function FontDropDown({
  ...
  const currentLabel =
    style === 'font-family'
      ? FONT_FAMILY_OPTIONS.find(([option]) => option === value)?.[1] ?? value
      : value;
...
export default function ToolbarPlugin({
  ...
  const $updateToolbar = useCallback(() => {
    ...
    if ($isRangeSelection(selection)) {
      ...
      updateToolbarState(
        'fontFamily',
        $getSelectionStyleValueForProperty(selection, 'font-family', DEFAULT_SANS_FONT_FAMILY),
      );
      let matchingParent;
