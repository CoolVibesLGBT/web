...
import { vibesStateAtom } from '@/state/vibes';
...
export default function Vibes({ reels: initialReels, activeTab: _activeTab, onPostClick: _onPostClick }: ReelsProps) {
  const [feedState, setFeedState] = useAtom(vibesStateAtom);
  const allReels = feedState.items as Reel[];
  ...
  const cursor = feedState.cursor != null ? String(feedState.cursor) : '';
  ...
  useEffect(() => {
    if (initialReels && initialReels.length > 0 && feedState.items.length === 0) {
      setFeedState(prev => ({
        ...prev,
        items: initialReels,
        isLoading: false,
        hasMore: prev.cursor ? prev.hasMore : false
      }));
    }
  }, [initialReels, feedState.items.length, setFeedState]);
  ...
  const fetchVibesFromAPI = useCallback(async (loadMore = false) => {
    try {
      ...
      const cursorString = nextCursor != null ? String(nextCursor) : '';
      const hasMorePosts = cursorString !== '' && cursorString !== '0' && cursorString !== 'null' && cursorString !== 'undefined';
      console.log('New cursor received:', cursorString);

      setFeedState(prev => ({
        ...prev,
        items: loadMore ? [...prev.items, ...newReels] : newReels,
        cursor: cursorString !== '' ? cursorString : null,
  ...
  // İlk yükleme
  useEffect(() => {
    if (!initialReels && feedState.items.length === 0) {
      fetchVibesFromAPI(false);
    } else {
      setFeedState(prev => ({ ...prev, isLoading: false }));
    }
     
  }, [initialReels, feedState.items.length, fetchVibesFromAPI, setFeedState]); // İlk yükleme
